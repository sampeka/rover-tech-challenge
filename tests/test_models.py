...
class TestCoordinate:
    def test_add_movement(self):
        ...
        coordinate += Movement(angle=90, delta=1)

        assert coordinate.y == 1
        assert coordinate.x == 0

        coordinate += Movement(angle=270, delta=1)

        assert coordinate.y == 0
        assert coordinate.x == 0

        coordinate += Movement(angle=0, delta=1)

        assert coordinate.y == 0
        assert coordinate.x == 1

        coordinate += Movement(angle=180, delta=1)
...
class TestGrid:
    ...
    def test_is_within_bounds(self, x, y, within_bounds):
        ...
        assert grid.is_within_bounds(position) == within_bounds
...
class TestRover:
    ...
    def test_move(self):
        grid = Grid(5, 5)
        rover = Rover(id='1', angle=90, position=Coordinate(0, 0))
        rover.move(MovementCommand.M, grid=grid)
        assert rover.position == Coordinate(0, 1)

    def test_move_outside_of_bounds(self):
        grid = Grid(5, 5)
        rover = Rover(id='1', angle=180, position=Coordinate(0, 0))

        with pytest.raises(exceptions.CollisionDetectedError):
            rover.move(MovementCommand.M, grid=grid)

        assert rover.position == Coordinate(0, 0)
